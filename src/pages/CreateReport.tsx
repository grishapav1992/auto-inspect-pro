...
const TIRE_TAGS = [
  { id: "tire_wear_uneven", label: "Неравномерный износ", emoji: "•" },
  ...
  { id: "tire_season", label: "Не по сезону", emoji: "•" },
...
const RIM_TAGS = [
  ...
  { id: "rim_bent", label: "Погнут диск", emoji: "•" },
...
const BRAKE_TAGS = [
  { id: "brake_pad_low", label: "Колодки изношены", emoji: "•" },
  { id: "brake_disc_wear", label: "Износ тормозного диска", emoji: "•" },
  { id: "brake_disc_groove", label: "Борозды на диске", emoji: "•" },
  ...
  { id: "handbrake_fault", label: "Ручник не держит", emoji: "•" },
] as const;
...
const SUSPENSION_TAGS = [
  ...
  { id: "spring_sag", label: "Просевшая пружина", emoji: "•" },
  { id: "ball_joint_play", label: "Люфт шаровой опоры", emoji: "•" },
  { id: "tie_rod_play", label: "Люфт рулевого наконечника", emoji: "•" },
  { id: "bushing_worn", label: "Износ сайлентблоков", emoji: "•" },
  ...
  { id: "alignment_off", label: "Нарушен сход-развал", emoji: "•" },
] as const;
...
const INTERIOR_DAMAGE_TAGS = [
  ...
  { id: "dashboard_scratch", label: "Царапины на торпедо", emoji: "•" },
  { id: "dashboard_crack", label: "Трещина торпедо", emoji: "•" },
  { id: "headliner_sag", label: "Провис потолка", emoji: "•" },
...
const FUNCTION_TAGS = [
  { id: "ac_fault", label: "Кондиционер / климат", emoji: "•" },
  ...
  { id: "sunroof_fault", label: "Люк", emoji: "•" },
] as const;
...
const CreateReport = () => {
  ...
  // Resolve IDB media URLs when loading an existing draft
  useEffect(() => {
    if (!existingDraft) return;
    let cancelled = false;
    loadDraftWithMedia(existingDraft).then((resolved) => {
      if (cancelled) return;
      // Update all photo arrays with resolved object URLs
