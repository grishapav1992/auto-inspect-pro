      ...
      >
        ...
        {isSelectMode && (
          <>
            ...
            {/* Show note & element only for photos with data */}
            {!item.children && (() => {
              ...
              const btnColor = isDraft
                ? "bg-warning text-warning-foreground"
                : "bg-success text-success-foreground";
              const elLabel = elementTypes?.find(et => et.id === item.inspection!.elementType)?.label;
              return (
                <>
                  ...
                  {elLabel && (
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent px-2 pb-1.5 pt-4 pointer-events-none">
                      <span className="text-[11px] font-medium text-white/90 leading-tight truncate block">{elLabel}</span>
                    </div>
                  )}
                </>
              );
